...
class ActionListView(LoginRequiredMixin, View):
    def get(self, request: HttpRequest, *args: tuple, **kwargs: dict) -> HttpResponse:
        action_logs: list[ActionLog] = ActionLog.find(ActionLog.user_id == request.user.id).sort_by('date').all()
    ...
    def post(self, request: HttpRequest, *args: tuple, **kwargs: dict) -> HttpResponse:
        action_log = ActionLog(user_id=request.user.id, **json.loads(request.body.decode('utf-8')))
        action_log.save()
